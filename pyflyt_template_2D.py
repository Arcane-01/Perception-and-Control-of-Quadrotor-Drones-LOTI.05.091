...
class CustomController(ControlClass):
	...
	def step(self, state: np.ndarray, setpoint: np.ndarray):
		...
		target_yaw_rate = 0.0
		target_z = setpoint[-1]
		output = np.array([*target_velocity[:2], target_yaw_rate, target_z])
		return output
	...
	def apf(self, observation, position_setpoint):
		...
		k_att = .5
		u_att = k_att * (goal[:2] - linear_position[:2])

		v = u_att

		return np.array(v)	# (vx, vy)
...
env.drones[0].register_controller(
	controller_constructor=CustomController, controller_id=8, base_mode=4
)
env.set_mode(8)
setpoint = np.array([1.0, 2.0, 0.0, start_pos[-1, -1]]) # (x, y, yaw, start_pos_z)
